...
class BagumumDataBarangController extends Controller
{
    ...
    public function index(){
        ...
        return view('bagumum.dataBarang', compact('data_barang'));
    }
    ...
    public function cetakTahunan(Request $request){
        ...
        $pdf = PDF::loadview('bagumum.laporan_barang_pdf',['data_barang'=>$data_barang, 'data_status'=>$request->post('tahunan')]);
    ...
    public function cetakBulanan(Request $request){
        ...
        $data_barang = DB::table('barang')->where('created_at', 'like', $parameter)->get();
        $pdf = PDF::loadview('bagumum.laporan_barang_pdf',['data_barang'=>$data_barang,'data_status'=>$bulan." ".$myArray[1]]);
    	return $pdf->download('laporan-data-barang.pdf');
    }
    public function cetakTanggal(Request $request){
        ...
        $bulan = $this->getMonth($myArray[0]);
        $data_barang = DB::table('barang')->where('created_at', 'like', $parameter)->get();
        $pdf = PDF::loadview('bagumum.laporan_barang_pdf',['data_barang'=>$data_barang,'data_status'=>$myArray[1]." ".$bulan." ".$myArray[1]]);
    	return $pdf->download('laporan-data-barang.pdf');
    }

    public function tambah(){
        return view('bagumum.tambahBarang');
    }
    public function ubah($id){
        $data_barang = DB::table('barang')->where('b_id',$id)->get();
        return view('bagumum.ubahBarang', compact('data_barang'));
    }
    public function lihat($id){
        $data_barang = DB::table('barang')->where('b_id',$id)->get();
        return view('bagumum.lihatBarang', compact('data_barang'));
    }
    public function prosesTambah(Request $request){
        ...
        $barang->save();
        return redirect(url('/bagumum/barang'));
    }
