...
class AdminDataInventarisController extends Controller
{
    ...
    public function index(){
        ...
        return view('admin.dataInventaris', compact('data_inventaris'));
    }
    ...
    public function cetakTahunan(Request $request){
        ...
        $pdf = PDF::loadView('admin.laporan_inventaris_pdf',['data_inventaris'=>$data_inventaris, 'data_status'=>$request->post('tahunan')]);
    ...
    public function cetakBulanan(Request $request){
        ...
        $data_inventaris = DB::table('inventaris')->where('created_at', 'like', $parameter)->get();
        $pdf = PDF::loadView('admin.laporan_inventaris_pdf',['data_inventaris'=>$data_inventaris,'data_status'=>$bulan." ".$myArray[1]]);
    	return $pdf->download('laporan-data-inventaris.pdf');
    }
    public function cetakTanggal(Request $request){
        ...
        $bulan = $this->getMonth($myArray[0]);
        $data_inventaris = DB::table('inventaris')->where('created_at', 'like', $parameter)->get();
        $pdf = PDF::loadView('admin.laporan_inventaris_pdf',['data_inventaris'=>$data_inventaris,'data_status'=>$myArray[1]." ".$bulan." ".$myArray[1]]);
    	return $pdf->download('laporan-data-inventaris.pdf');
    }

    public function tambah(){
        return view('admin.tambahInventaris');
    }
    public function ubah($id){
        $data_inventaris = DB::table('inventaris')->where('i_id',$id)->get();
        return view('admin.ubahInventaris', compact('data_inventaris'));
    }
    public function lihat($id){
        $data_inventaris = DB::table('inventaris')->where('i_id',$id)->get();
        return view('admin.lihatInventaris', compact('data_inventaris'));
    }
