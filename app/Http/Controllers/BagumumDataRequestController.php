...
class BagumumDataRequestController extends Controller
{
    ...
    public function index(){
        ...
        ->get();
        return view('bagumum.dataRequest', compact('data_request'));
    }
    ...
    public function lihat($id){
        ...
        ->get();
        return view('bagumum.lihatRequest', compact('data_request'));
    }
    ...
    public function prosesKonfirmasi(Request $request){
        ...
        if($jenis == 2){
            ...
            if($data_barang[0]->b_stock < $data_request[0]->rb_jumlah){
                echo "<script>alert('Gagal, Stock barang tidak cukup')
                    ;window.location='".url('/bagumum/request')."'</script>";
            }else{
        ...
        return redirect(url('/bagumum/request'));
    }
    ...
    public function cetakTahunan(Request $request){
        ...
        ->get();
        $pdf = PDF::loadview('bagumum.laporan_request_pdf',['data_request'=>$data_request, 'data_status'=>$request->post('tahunan')]);
    ...
    public function cetakBulanan(Request $request){
        ...
        ->where('request_barang.created_at', 'like', $parameter)
        ->get();
        $pdf = PDF::loadview('bagumum.laporan_request_pdf',['data_request'=>$data_request,'data_status'=>$bulan." ".$myArray[1]]);
    	return $pdf->download('laporan-data-request.pdf');
    }
    public function cetakTanggal(Request $request){
        ...
        ->where('request_barang.created_at', 'like', $parameter)
        ->get();
        $pdf = PDF::loadview('bagumum.laporan_request_pdf',['data_request'=>$data_request,'data_status'=>$myArray[1]." ".$bulan." ".$myArray[1]]);
    	return $pdf->download('laporan-data-request.pdf');
    }
